...
export function catmullRom(controlPoints, samplesPerSegment) {
...
function catmullRomSegment(p0, p1, p2, p3, out, resolution, alpha = 0.5) {
    const k0 = 0;
...
function remap(a, b, t, t0, t1) {
    const weight = (t - t0) / (t1 - t0);
    return new Vec3(a.x + (b.x - a.x) * weight, a.y + (b.y - a.y) * weight, a.z + (b.z - a.z) * weight);
}
